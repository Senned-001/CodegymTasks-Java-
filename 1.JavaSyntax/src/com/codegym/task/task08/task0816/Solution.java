...
public class Solution {
    ...
    public static void removeAllSummerPeople(HashMap<String, Date> map) throws ParseException {
        DateFormat df = new SimpleDateFormat("MMMMM", Locale.ENGLISH); //zadaetsa format vozvrachaebmoy dati v string
        ...
        for (HashMap.Entry<String, Date> ent : map.entrySet()) {
            ...
            if (df.format(ent.getValue()).equals("June")) keysToRemove.add(ent.getKey()); //sravnivaem vozvrachaemuyu datu v formate nazvaniya mecyatca
            if (df.format(ent.getValue()).equals("July")) keysToRemove.add(ent.getKey());  //i zanosim v kuchu dly dalneishego udaleniya
            if (df.format(ent.getValue()).equals("August")) keysToRemove.add(ent.getKey());

        }
        map.keySet().removeAll(keysToRemove); //udalenie kluchey kotorie est v kuche
